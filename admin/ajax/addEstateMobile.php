...
if ($_SERVER["REQUEST_METHOD"] == 'POST') {
    session_start();
        if (
            ...
            $select = mysqli_query($conn,"SELECT * FROM admin WHERE admin.adminUserName='$userName' AND admin.adminPassword='$va_password'");
            if(mysqli_num_rows($select)==1){
                $rowAdmin = mysqli_fetch_assoc($select);
           ...
           $adminId = mysqli_real_escape_string($conn,$_SESSION['id']);
            ...
            estate_renter,estate_unitFloor,estate_masterOner
            )
         VALUES (
